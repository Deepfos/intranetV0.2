...
def upload_design(request):
    if request.method == 'POST':
        ...
        docente_rut = request.POST.get('docente')
        alumno_rut = request.POST.get('alumno')
        curso_codigo = request.POST.get('curso')
        beneficio_id = request.POST.get('beneficio')

        # Parsear la fecha de expiración desde el formato de formulario
        ...
        docente = Docente.objects.get(rut=docente_rut)
        ...
        return redirect('core/diseno.html')  

    # Obtener datos necesarios para el formulario (docentes, alumnos, cursos, beneficios)
    docentes = Docente.objects.all()
    alumnos = Alumno.objects.all()
    cursos = Curso.objects.all()
    beneficios = Beneficio.objects.all()

    return render(request, 'nombre_de_tu_template.html', {'docentes': docentes, 'alumnos': alumnos, 'cursos': cursos, 'beneficios': beneficios})
...
#validación tótem INDEX
def validacion_totem(request):
    if request.method == 'POST':
        totem_code = request.POST.get('totem_code')
        try:
            totem = Totem.objects.get(codigo=totem_code)
            messages.success(request, 'Tótem validado exitosamente.')
        except Totem.DoesNotExist:
            messages.error(request, 'El código del Tótem no es válido.')
...
def agregarTotemsDocente(request):
    ...
    if request.method == 'POST':
        ...
        if codigo and motivo and docente and curso and alumno and beneficio:
            # Obtener el objeto Curso y Alumno relacionados
            curso_obj = Curso.objects.get(codigo=curso)
            alumno_obj = Alumno.objects.get(rut=alumno)
...
def validar_totem(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            # Combine the three parts of the code
            totem_code = f"{form.cleaned_data['code_part1']}-{form.cleaned_data['code_part2']}-{form.cleaned_data['code_part3']}"

            # Query the Totem model to check if the code exists
            try:
                totem = Totem.objects.get(codigo=totem_code)
                # If the totem exists, pass the totem data to the template
                return render(request, 'core/index.html', {'totem_data': totem})
            except Totem.DoesNotExist:
                # If the totem does not exist, handle accordingly (e.g., show an error message)
                return render(request, 'core/index.html', {'error_message': 'Totem not found'})
    else:
        form = TotemValidationForm()

    return render(request, 'core/core/index.html', {'form': form})
